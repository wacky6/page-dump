...
function headersToObject(ha) {
    var obj = {}
    ha.forEach(function($){ obj[$.name.toLowerCase()]=$.value })
    return obj
}

function printResult() {
    var _rr = rr.map(function($){
        var req = $.request
        var res = $.response
        if (!res)
            res = {
        ...
        return {
            ...
            error:      res.error,
            reqTime:    new Date(req.time).getTime(),
            resTime:    new Date(res.time).getTime(),
